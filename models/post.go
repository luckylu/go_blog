...
type Posts struct {
	...
	Title     string      `orm:"size(128);column(title)"`
	Content   string      `orm:"type(longtext);column(content)"`
...
func GetPostsById(id int) (v *Posts, err error) {
	o := orm.NewOrm()
	v = &Posts{Id: id}
	if err = o.QueryTable(new(Posts)).Filter("Id", id).RelatedSel().One(v); err == nil {
		return v, nil
	}
	return nil, err
}
